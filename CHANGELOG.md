...
## [0.0.5] - 2026-09-06 (待发布)

### 变更
- **模型 tab 布局重构**：左侧（模型列表 + 基础信息 + 推测解码） | 右侧（全部给模型参数）
- 基础信息改 2 列紧凑布局（6 行 → 3 行 + 启用）
- 推测解码默认展开（一般不用改）
- 模型参数面板从 ~350px 提升到 **600+px**（占满右侧栏所有高度）
- 顶部保存条缩短（更紧凑）
- 模型编辑面板新增 `base_url` 和 `api_key` 字段（可选）
  - `api_key` 显示为 `****` 密码输入框，支持眼睛图标切换显示/隐藏
  - 默认值为空（留空 = 用本机 `llama_dir` / 无密钥）
- 监控窗口新增 `base_url` / `api_key` 模型状态信息
- 首次运行向导：空配置时弹窗引导用户设置
...
- **新规则**：以后不会自动发布
  - 修改 → commit + push (main)
  - 想发布时显式说 "发布 v0.0.x"
  - 先跑 `pre_publish_check.py` 确认就绪
  - 打 tag 触发 GitHub Actions 自动打包 + 发布
